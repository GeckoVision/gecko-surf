...
def test_clean_cross_api_plan_validates_against_official_arazzo_schema() -> None:
    ...
    doc = _cross_plan_doc(
        sources={
            "txline": "https://api.txodds.example/openapi.yaml",
            "market": "https://markets.example.com/openapi.json",
        }
    )
    assert _errors(doc) == []
...
def test_clean_export_shape_matches_the_plan() -> None:
    ...
    wf = doc["workflows"][0]
    steps = wf["steps"]
    assert [s["x-gecko-surface-id"] for s in steps] == ["txline", "market"]
    assert steps[-1]["x-gecko-operation-id"] == "openMarket"
    # two sourceDescriptions -> operationId MUST be the qualified runtime expression
    assert len(doc["sourceDescriptions"]) == 2
    assert steps[-1]["operationId"].startswith("$sourceDescriptions.market.")
    assert steps[-1]["operationId"].endswith(".openMarket")
...
def test_single_source_uses_a_bare_operation_id() -> None:
    ...
    ws = Workspace(graphs=(tx.graph,))
    plan = cross_plan(ws, "txline", "getApiOddsUpdatesFixtureid", set())
    assert plan is not None
    doc = to_arazzo(plan, graphs=(tx.graph,))
    assert _errors(doc) == []
    assert all(
...
def test_cross_edge_carries_provenance_basis_confidence_and_the_gate() -> None:
    ...
    param = next(p for p in target["parameters"] if p["name"] == "fixture_ref")
    prov = param["x-gecko-provenance"]
    assert prov["provenance"] == "DECLARED"
    assert prov["basis"] == "declared:fixture"
    assert prov["confidence"] == "high"
    assert prov["sourceSurfaceId"] == "txline"
    assert prov["sourceField"] == "FixtureId"
    ...
    assert param["value"] == f"$steps.{supplier['stepId']}.outputs.FixtureId"
    assert supplier["outputs"]["FixtureId"] == "$response.body#/FixtureId"
...
def test_the_same_chain_unpoisoned_exports_a_valid_runnable_workflow() -> None:
    """The falsifiable hinge: flip the poison off and the SAME chain exports clean."""
    ...
    assert is_executable(doc)
    assert any(
        s["x-gecko-operation-id"] == _EXIT_OP for s in doc["workflows"][0]["steps"]
    )
...
def test_spec_authored_examples_never_leak_into_the_export() -> None:
    ...
    doc = json.dumps(_cross_plan_doc())
...
def test_export_is_deterministic() -> None:
    a = json.dumps(_cross_plan_doc(), sort_keys=True)
    b = json.dumps(_cross_plan_doc(), sort_keys=True)
    assert a == b


def test_unknown_source_url_is_marked_not_invented() -> None:
    doc = _cross_plan_doc()  # no `sources` given
...
def test_cli_export_arazzo_prints_a_document_and_exits_nonzero_on_refusal(
    ...
    assert (
        main(["export-arazzo", str(TXLINE), "--op", "getApiOddsUpdatesFixtureid"]) == 0
    )
    ...
    assert main(["export-arazzo", str(TXLINE), "--op", "doesNotExist"]) == 3
...
def test_mcp_export_arazzo_method() -> None:
    ...
    client = AgentApiClient(
        str(TXLINE), surface_id="txline", declared_hints=TXLINE_HINTS
    )
    doc = McpSurface(client).export_arazzo("getApiOddsUpdatesFixtureid")
    assert doc["arazzo"] == ARAZZO_VERSION
    assert is_executable(doc)
