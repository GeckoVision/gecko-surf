...
def test_two_accounts_of_one_mint_declaring_different_decimals_refuse() -> None:
    """DEFECT 1 — the silent 1000x under-report.

    The pre/post check is per-ACCOUNT, so two DIFFERENT accounts of the same
    ``(owner, mint)`` could each be internally consistent while declaring 6 and 9
    decimals. ``outflows()`` then summed their raw deltas and stamped whichever decimals
    it saw last onto the total: 50_000_000 raw rendered as "0.050000000" instead of
    "50.000000" — a 1000x under-report against any ui-denominated cap.

    A mint's scale is immutable on chain, so a disagreement ANYWHERE in the report means
    one of the readings is wrong. Same mint must mean one decimals value.
    """
    report = _report(
        _value(
            [
                _balance(index=3, amount="100000000", decimals=6),
                _balance(index=7, amount="100000000", decimals=9),
            ],
            [
                _balance(index=3, amount="75000000", decimals=6),
                _balance(index=7, amount="75000000", decimals=9),
            ],
        )
    )
    assert report.status == "unmeasurable"
...
def test_two_owners_of_one_mint_declaring_different_decimals_refuse() -> None:
    """The same disagreement across OWNERS. The scale belongs to the mint, not to the
    holder, so a per-(owner, mint) check would still miss this one."""
    other_owner = "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM"
    report = _report(
        _value(
            [
                _balance(index=3, amount="100000000", decimals=6),
                _balance(index=7, owner=other_owner, amount="100000000", decimals=9),
            ],
            [
                _balance(index=3, amount="75000000", decimals=6),
                _balance(index=7, owner=other_owner, amount="75000000", decimals=9),
            ],
        )
    )
    assert report.status == "unmeasurable"
...
def test_two_accounts_of_one_mint_agreeing_on_decimals_still_measure() -> None:
    """The guard must refuse a DISAGREEMENT, not multi-account mints as such."""
    report = _report(
        _value(
            [
                _balance(index=3, amount="100000000", decimals=6),
                _balance(index=7, amount="5000000", decimals=6),
            ],
            [
                _balance(index=3, amount="75000000", decimals=6),
                _balance(index=7, amount="0", decimals=6),
            ],
        )
    )
    assert report.status == "measured"
    (outflow,) = report.outflows()
    assert (outflow.raw, outflow.decimals, outflow.ui) == (30_000_000, 6, "30.000000")


def test_a_unicode_digit_amount_refuses_rather_than_raising() -> None:
    """DEFECT 2 — ``str.isdigit()`` is True for "\u00b2" but ``int()`` rejects it, so the
    ValueError escaped ``parse_token_deltas`` (and ``simulate``) instead of becoming the
    documented ``malformed-balance`` refusal. All RPC output is untrusted input."""
    report = _report(_value([_balance(amount="\u00b2")], [_balance(amount="75000000")]))
    assert report.status == "unmeasurable"
    assert [r.reason for r in report.refusals] == ["malformed-balance"]


def test_a_unicode_digit_amount_does_not_escape_simulate() -> None:
    receipt = simulate(
        PLAN,
        rpc_url="https://rpc.example/x",
        rpc_call=_rpc(_value([_balance(amount="\u00b2")], [_balance(amount="1")])),
...
# --- DEFECT 3: absent vs [] vs null are three different facts -------------------------
#
# Measured against api.mainnet-beta.solana.com on 2026-08-12: a tx that MOVES tokens
# returns populated arrays; a tx that touches NO token account (let_me_buy initialize)
# returns EMPTY ARRAYS. ``null`` was produced by neither.


def test_absent_token_balance_keys_are_not_tracked() -> None:
    """Shape 1 — key ABSENT: the stock ``simulateTransaction`` case. ``None``, not zero."""
    assert parse_token_deltas(_value(None, None)) is None


def test_empty_token_balance_arrays_are_a_measured_zero_with_no_refusal() -> None:
    """Shape 2 — ``[]``: a POSITIVE fact from the node about a real transaction. This is
    exactly what ``initialize`` returns, and it must stay signable."""
    report = _report(_value([], []))
    assert report.status == "measured"
    assert report.refusals == ()
    assert report.movements == ()
    assert report.outflows() == ()
    assert report.net_raw(mint=USDC, owner=OWNER) == 0


@pytest.mark.parametrize(
    "pre,post",
    [(None, []), ([], None), (None, None)],
    ids=["pre-null", "post-null", "both-null"],
)
def test_null_token_balances_earn_their_own_distinct_refusal(
    pre: Any, post: Any
) -> None:
    """Shape 3 — ``null``: the node DECLINING to provide the data, which is not the node
    stating nothing moved. We cannot tell "no token movement" from "not computed", and an
    SPL spend reading as zero outflow defeats the cap — so we refuse, under a reason of
    its own rather than reusing ``malformed-balance``."""
    value: dict[str, Any] = {"err": None, "logs": [], "unitsConsumed": 86_669}
    value["preTokenBalances"] = pre
    value["postTokenBalances"] = post
    report = _report(value)
    assert report.status == "unmeasurable"
    assert "balances-null" in {r.reason for r in report.refusals}
    assert "balances-null" in TOKEN_DELTA_REFUSALS
    with pytest.raises(TokenDeltaUnmeasurable):
        report.outflows()


def test_null_balances_are_not_collapsed_into_the_malformed_reason() -> None:
    """Distinct on purpose: "the node declined" and "the payload is the wrong shape" are
    different operator actions, and only one of them means retry elsewhere."""
    value: dict[str, Any] = {"preTokenBalances": None, "postTokenBalances": None}
    report = _report(value)
    assert [r.reason for r in report.refusals] == ["balances-null"]


def test_simulate_refuses_on_null_balances_but_measures_empty_ones() -> None:
    """The ``initialize``/``make_purchase`` pair, at the Receipt seam."""
    empty = simulate(
        PLAN,
        rpc_url="https://rpc.example/x",
        rpc_call=_rpc(_value([], [])),
        build_call=_build_ok,
        network=UNKNOWN_NETWORK,
    )
    assert empty.token_delta is not None
    assert empty.token_delta.status == "measured"
    assert empty.token_delta.refusals == ()

    nulled = simulate(
        PLAN,
        rpc_url="https://rpc.example/x",
        rpc_call=_rpc({"preTokenBalances": None, "postTokenBalances": None}),
        build_call=_build_ok,
        network=UNKNOWN_NETWORK,
    )
    assert nulled.token_delta is not None
    assert [r.reason for r in nulled.token_delta.refusals] == ["balances-null"]
