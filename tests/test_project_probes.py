...
def test_both_token_accounts_are_the_right_owners_ata() -> None:
    case = case_for("make_purchase")
    assert case.accounts["sender_token_account"] == derive_ata(
        BUYER, USDC, token_program=TOKEN_PROGRAM
    )
    assert case.accounts["recipient_token_account"] == derive_ata(
        AUTHORITY, USDC, token_program=TOKEN_PROGRAM
    )
    assert (
