...
echo ""
echo "Verifying /comprehend/servable after a deploy — READ THIS BEFORE PANICKING:"
echo "  An UNAUTHENTICATED probe returns 404 BY DESIGN. The route checks the token"
echo "  before it parses the body, so a 404 there means nothing at all — it is the"
echo "  same answer a stranger gets, and the same answer a missing route gives."
echo "  Two checks that actually distinguish the cases:"
echo ""
echo "  1. Is the ROUTE deployed? Compare the 404 BODY, not the status:"
echo "       curl -s -X POST https://<host>/comprehend/servable -d '{}'"
echo "     {\"error\":\"not found\"}   -> our handler ran: route IS deployed, token rejected"
echo "     {\"detail\":\"Not Found\"}  -> Starlette: the route is NOT in the running image"
echo ""
echo "  2. Is the TOKEN wired? Send it and expect 400 (\"missing 'url'\"). Keep the"
echo "     secret out of argv and shell history by using a 600-mode curl config:"
echo "       umask 077; printf 'header = \"X-Gecko-Servable-Token: %s\"\\n\" \"\$TOKEN\" > /tmp/c"
echo "       curl -s --config /tmp/c -X POST https://<host>/comprehend/servable -d '{}'"
echo "       shred -u /tmp/c"
echo ""
