...
@dataclass
class BearerSession:
    """Injects the PAT as a bearer token. The real User-Agent is what gets the stdlib
    client past Colosseum's Cloudflare WAF (which 403s the default Python-urllib UA)."""
    ...
    def auth_headers(self) -> dict[str, str]:
        return {
            ...
            "User-Agent": "gecko/0.2 (+https://geckovision.tech)",
        }
...
def main() -> int:
    ...
    print(f"Colosseum Copilot — {len(client.list_tools())} first-call-correct tools ready.")
    print("PAT injected at call time, hidden from the agent, sent only to Colosseum.")
    print("Add it:  claude mcp add --transport http colosseum http://127.0.0.1:8000/mcp")
