...
243 modules in `gecko/` (subpackages included), from their own docstrings.
...
| `sandbox.rehearse` | The whole purchase, rehearsed on a fork — and JUDGED by what actually moved. | 7 · `fork_lane`, `sandbox.agents`, `sandbox.deliver` +4 |
...
| `sandbox.surfnet` | Prove the RPC is a surfnet BEFORE any key material can exist. | 9 · `sandbox.agents`, `sandbox.cheatcodes`, `sandbox.deliver` +6 |
...
| `whirlpool_venue` | Find the pool that converts one mint into another — and make each candidate prove it. | 2 · `pay_route`, `providers.whirlpool` |
