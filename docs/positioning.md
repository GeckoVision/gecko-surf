...
started spending real money through the engine (64 mainnet transactions; of the 49 with
a compute prediction made before signing, 47 matched the chain to the unit; the enclave
...
> against a simulation, and hand your wallet something checked. 64 mainnet
> transactions, 47 of 49 cost predictions exact, the latest a stablecoin swap and a
> purchase with the network fee paid by a relay and the buyer's wallet holding no SOL —
> and in every one, the key was never ours.
