...
Two skills for **API providers** who want agents to actually *use* their API:

| Skill | What it does | Status |
|---|---|---|
| [`api-agent-ready`](skills/api-agent-ready/SKILL.md) | Comprehend the API with `gecko` → emit the agent-native breadcrumbs → serve the full surface over MCP with a one-click add → make it discoverable. **Leaves the provider's own MCP intact.** | comprehend + serve **Live**; artifacts + discoverability **Building** |
| [`x402-payai-setup`](skills/x402-payai-setup/SKILL.md) | Wire x402 micropayments onto the provider's API via **PayAI** — point the agent-facing tools at the provider's own x402 endpoint. **The provider keeps 100%.** | handshake/offline stub **Building (Pattern B)**; live settlement **Building / founder-gated** |
